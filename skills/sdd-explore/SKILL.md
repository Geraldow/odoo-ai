...
### Step 1b: Odoo Detection + Hub Classification (run before investigation)

```bash
ls __manifest__.py 2>/dev/null || find . -name "__manifest__.py" -maxdepth 2 | head -1
```

**If Odoo project detected:**
- Read `__manifest__.py` → Odoo version + module name
- Note version-specific traps (v17: `attrs` removed, `name_get` deprecated; v18: same + `name_get` removed)

**Hub classification — determine which hub applies to this exploration:**

```
¿De qué trata la exploración?
├── Código: modelos, vistas, ORM, campos, fix, feature, OWL, tests, templates
│   → Hub: odoo-development
│   → Acción: usar odoo-source para Module Intelligence Report del módulo afectado
│
├── VCS / estructura / convenciones / SSH / DB / orientación general
│   → Hub: odoo-contribute
│   → Acción: identificar el plugin correspondiente (odoo-commit, odoo-oca, odoo-ops, etc.)
│
└── Nuevo módulo desde cero (scaffold + código)
    → Ambos hubs en orden:
       1. odoo-contribute (odoo-module: estructura, odoo-oca: naming)
       2. odoo-development (fhidalgo: templates de código)
```
