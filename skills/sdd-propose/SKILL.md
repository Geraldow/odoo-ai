...
### Step 0: Odoo Detection + Hub Classification
...
- Read `__manifest__.py` → extraer versión Odoo
- Clasificar el cambio propuesto:

```
¿Qué tipo de cambio es?
├── Código (modelos, vistas, ORM, feature, fix)
│   → Hub: odoo-development
│   → El Approach del proposal debe referenciar patrones de odoo-development
│
├── VCS / estructura / convenciones / SSH / DB
│   → Hub: odoo-contribute
│   → El Approach debe referenciar el plugin correspondiente
│
└── Nuevo módulo desde cero
    → Ambos hubs: odoo-contribute (scaffold + OCA) + odoo-development (código)
```

- Incluir la versión Odoo detectada en el campo `## Approach` del proposal
- Si ya viene de sdd-explore, el hub ya está clasificado — no re-detectar, solo confirmar
