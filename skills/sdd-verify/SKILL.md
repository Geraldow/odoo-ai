...
├── __manifest__.py present → Odoo project (see Odoo test commands below)
├── package.json → scripts.test
├── pyproject.toml / pytest.ini → pytest
├── Makefile → make test
└── Fallback: ask orchestrator

ODOO TEST COMMANDS (when __manifest__.py detected):
├── Unit/integration: python odoo-bin -u {module} --test-enable -d {db} --stop-after-init
├── Pytest style:     pytest addons/{module}/tests/ --odoo-database={db}
├── Specific class:   python odoo-bin -u {module} --test-tags={TestClass} -d {db} --stop-after-init
└── Always confirm db name with user before running against any database
