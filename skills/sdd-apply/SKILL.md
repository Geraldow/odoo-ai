...
### Step 0a: Modo Aprendizaje (OBLIGATORIO — ejecutar PRIMERO)

Preguntar UNA SOLA VEZ antes de escribir cualquier código:
...
Si viene de `sdd-ff` o `sdd-new` y ya se preguntó → **NO preguntar de nuevo**, respetar la respuesta anterior.

---

### Step 0b: Odoo Detection (MANDATORY — before anything else)

```bash
# 1. Detect Odoo project
ls __manifest__.py 2>/dev/null || find . -name "__manifest__.py" -maxdepth 2 | head -1
...
**If `__manifest__.py` found → Odoo project detected:**
- Read `__manifest__.py` → extract Odoo version (e.g. `18.0.x.x.x` → v18)
- Load `odoo-development` skill for ALL code patterns (models, views, security, tests)
- Apply branch safety: must be on `st_*` or `st_produccion`. If `produccion`/`db_*` → STOP and ask authorization
- After implementation: save to engram with `odoo/dev/{module}/{feature}` convention

**If no `__manifest__.py` → non-Odoo project:**
- Proceed with standard implementation below
...
## Step 5: Generate Closure Report (MANDATORY — last step before done)

When ALL assigned tasks are complete and marked `[x]`:

1. Read the `sdd-report` skill: `~/.claude/skills/sdd-report/SKILL.md`
2. Follow its structure exactly to generate the full closure report
3. Output the report directly as rendered markdown — NEVER in a code block
4. The report is the LAST thing returned — after code, after summary, after engram save

If only SOME tasks are complete (partial batch), skip this step — report only when the full change is done.
